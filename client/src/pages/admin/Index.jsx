          ...
          {users.length === 0 || users[0]===null ? (
            ...
            <ul className="divide-y w-container mx-auto divide-gray-200">
              {users && users.length > 0 && users.map(user => (
                <li key={user && user.id} className="py-4 flex justify-between items-center">
                  <div className=' '>
                    <div>
                      <p className="text-lg font-semibold">Name: {user && user.name ? user.name : ''}</p>
                      <p className="text-gray-500">Email: {user && user.email ? user.email : ''}</p>
                      <p className="text-gray-500">Role: {user && user.role ? user.role : ''}</p>
                    </div>
                    <div className="text-gray-500">
                      <p>User ID: {user && user._id ? user._id : 'N/A'}</p>
                    </div>
                  </div>
                  <div>
                    <button className='bg-orange-300 px-4 py-3 rounded text-white font-DM text-base font-semibold' onClick={() => { handleUserDelete(user._id, user.role) }}>Delete User</button>
                  </div>
                </li>
              ))}
            </ul>
